...
import gsap from 'gsap';
import { motion, AnimatePresence } from 'framer-motion';
// Ensure you have this component or remove the import if not
import LightRays from './LightRays';
...
// --- Styles Moved Outside to Prevent Flash ---
const BENTO_STYLES = (glowColor) => `
  .bento-section {
    --glow-x: 50%;
    --glow-y: 50%;
    --glow-intensity: 0;
...
const ParticleCard = ({
    ...
    useEffect(() => {
        ...
        // Use GSAP Context for easy cleanup of all particle animations
        const ctx = gsap.context((self) => {
            ...
            const handleMouseEnter = () => {
                ...
                for (let i = 0; i < particleCount; i++) {
                    setTimeout(() => {
                        if (!isHoveredRef.current || !element) return;

                        // Dynamic particle creation logic
                        ...
                        // Auto-kill particle if we leave (cleanup handled by context mostly, but good to be explicit)
                        return particle;
                    }, i * 100);
                }

                if (enableTilt) {
                    gsap.to(element, { rotateX: 5, rotateY: 5, duration: 0.3, transformPerspective: 1000 });
                }
            };
        ...
        return () => ctx.revert(); // Powerful cleanup
    }, [disableAnimations, particleCount, glowColor, enableTilt, enableMagnetism]);
...
const GlobalSpotlight = ({ gridRef, disableAnimations = false, enabled = true, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = DEFAULT_GLOW_COLOR }) => {
    useEffect(() => {
        ...
        spotlight.style.cssText = `
            ...
            background: radial-gradient(circle, rgba(${glowColor}, 0.15) 0%, rgba(${glowColor}, 0.01) 65%, transparent 70%);
            z-index: 200; opacity: 0; transform: translate(-50%, -50%); mix-blend-mode: screen;
            top: 0; left: 0;
        `;
        document.body.appendChild(spotlight);

        // Optimization: Use Ticker instead of raw event listener for smoother performance
        let mouseX = 0, mouseY = 0, isVisible = false;

        const updateSpotlight = () => {
            if (!gridRef.current || !spotlight) return;

            const section = gridRef.current.closest('.bento-section');
            if(!section) return;

            const rect = section.getBoundingClientRect();
            const mouseInside = mouseX >= rect.left && mouseX <= rect.right && mouseY >= rect.top && mouseY <= rect.bottom;
            ...
            // Move spotlight efficiently
            spotlight.style.transform = `translate(${mouseX}px, ${mouseY}px) translate(-50%, -50%)`;

            // Calculate card glows (Throttle this if still laggy)
            const cards = gridRef.current.querySelectorAll('.card');
            const { proximity, fadeDistance } = calculateSpotlightValues(spotlightRadius);
            ...
            cards.forEach(card => {
                ...
                updateCardGlowProperties(card, mouseX, mouseY, glowIntensity, spotlightRadius);
            });
        };
        ...
        gsap.ticker.add(updateSpotlight); // Sync with refresh rate
...
const BentoCardGrid = ({ children, gridRef }) => (
    <div className="bento-section grid gap-2 p-3 w-[100vw] select-none relative"
        style={{ fontSize: 'clamp(1rem, 0.9rem + 0.5vw, 1.5rem)' }}
        ref={gridRef}>
        {children}
    </div>
...
const MagicBento = ({
    ...
    return (
        <>
            ...
            <BentoCardGrid gridRef={gridRef}>
                <div className={`grid gap-4 md:gap-8 w-full md:grid-cols-2 [grid-template-areas:'startup_startup''bplan_casestudy''pecharcha_pecharcha''event6_iplauction''workshop_workshop''pitchers_bizquiz'] max-w-6xl mx-auto mb-20`}>
                    {cardData.map((card, index) => {
                        const baseClassName = `card flex flex-col justify-between relative aspect-[4/3] min-h-[200px] max-h-[55vh] w-full max-w-full p-2 rounded-[20px] border border-solid font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${enableBorderGlow ? 'card--border-glow' : ''}`;

                        // Map index to grid areas
                        const areas = ['startup', 'bplan', 'casestudy', 'pecharcha', 'event6', 'iplauction', 'workshop', 'pitchers', 'bizquiz'];
                        const gridAreaClass = areas[index] ? `[grid-area:${areas[index]}]` : '';

                        const Wrapper = enableStars ? ParticleCard : 'div';
                        const wrapperProps = enableStars ? {
                            particleCount, glowColor, enableTilt, clickEffect, enableMagnetism, disableAnimations: shouldDisableAnimations
                        } : {};

                        return (
                            <Wrapper
                                key={index}
                                className={`${baseClassName} ${gridAreaClass} group`}
                                style={{
                                ...
                                {...wrapperProps}
                            >
                                <div className="absolute inset-0 bg-cover bg-no-repeat bg-center transition-transform duration-700 ease-out scale-100 group-hover:scale-110 group-hover:opacity-50"
                                     style={{ backgroundImage: `url(${card.image})` }} />
                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-60 transition-opacity duration-500" />

                                <div onClick={() => { if (isMobile) setSelectedCard(card) }}
                                     className="relative z-10 flex flex-col justify-between h-full p-1 md:p-5 cursor-pointer">
                                    <h3 className="text-xl md:text-4xl font-semibold">{card.title}</h3>
                                    <p className="mt-0 md:mt-2 hidden md:block md:text-lg opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-500">
                                        {card.description}
                                    </p>
            ...
            <AnimatePresence>
                {isMobile && selectedCard && (
                    ...
                    >
                         {/* Ensure LightRays exists or remove this block */}
                        <div className="absolute top-0 left-0 w-full h-full bg-[#00000] text-white p-6 md:p-10 shadow-2xl" onClick={(e) => e.stopPropagation()}>
                            <button onClick={closeOverlay} className="absolute top-4 right-4 text-[#fac176] text-3xl">×</button>
                            <div className="flex flex-col justify-center items-start w-full h-full overflow-y-auto">
                                <img src={selectedCard.image} alt={selectedCard.title} className="w-full h-64 object-contain mb-6 rounded-lg" />
                                <h2 className="text-4xl font-bold mb-4 mt-4">{selectedCard.title}</h2>
                                <p className="text-lg leading-relaxed opacity-90">{selectedCard.description}</p>
                            </div>
                        </div>
